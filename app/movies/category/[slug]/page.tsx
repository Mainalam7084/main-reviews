import { getMoviesByCategoryPaginated, CATEGORIES, type CategorySlug } from '@/lib/tmdb';
...
export default async function CategoryPage({ params }: PageProps) {
    ...
    const { results: movies, total_pages } = await getMoviesByCategoryPaginated(slug, 1);
    const accentColor = CATEGORY_COLORS[slug] || 'var(--primary)';

    let filterLanguage: string | undefined;
    if (config.type === 'language') {
        filterLanguage = (config as any).value;
    } else if (config.type === 'genre-lang') {
        filterLanguage = (config as any).value?.lang;
    }
            ...
            <div className="py-12 md:py-20 border-b-3 border-border bg-background relative overflow-hidden">
                ...
                <div className="relative z-10 max-w-5xl px-4 md:px-8">
                    ...
                    <p className="mt-6 text-xl md:text-2xl font-500 text-muted-foreground font-sans max-w-2xl border-l-4 border-foreground pl-6">
                        Explore the best movies in this collection.{' '}
                        <span className="text-foreground font-700">Premium cinema</span>, curated for you.
                    </p>
                </div>
            </div>
